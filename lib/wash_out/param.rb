module WashOut
  class Param
    ...
    def namespaced_type
      namespace = struct? ? 'tns' : 'xsd'
      namespace = self.source_class.xsi_namespace_value if self.source_class.present? && self.source_class.xsi_namespace_value.present?

      struct? ? "#{namespace}:#{basic_type}" : "#{namespace}:#{xsd_type}"
    end
